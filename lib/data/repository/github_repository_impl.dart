...
class GitHubRepositoryImpl implements GitHubRepository {
  ...
  Future<List<GitHubUserEntity>> fetchUsersByLocation(String location,
    ...
    final List<GitHubUserModel> userModels = await _gitHubService
        .fetchUsersByLocation(location, page: page, perPage: perPage);
    return userModels.map((model) => model.toEntity()).toList();
  }

  @override
  Future<List<GitHubUserEntity>> fetchUsersByFilter(
      {required String name, int minFollowers = 0, int minRepos = 0}) async {
    final List<GitHubUserModel> userModels =
        await _gitHubService.fetchUsersByFilter(name: name);
    return userModels.map((model) => model.toEntity()).toList();
  }
